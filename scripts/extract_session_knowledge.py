...
TECHNICAL_PATTERNS = [
    ...
    (
        ...
        "windows_encoding",
        "Use encoding='utf-8' for Windows compatibility",
    ),
    (
        r"errors\s*=\s*['\"]replace['\"]",
        "error_handling",
        "Use errors='replace' to handle unencodable chars",
    ),
    (r"2>NUL", "windows_redirect", "Use 2>NUL instead of 2>/dev/null on Windows"),
    # Architecture patterns
    (
        r"cli_executor\.extract\s*\(",
        "cli_extraction",
        "Use cli_executor.extract(prompt, text) for paper extraction",
    ),
    (
        r"call_with_prompt\s*\(",
        "cli_combined",
        "call_with_prompt() combines prompt and text differently",
    ),
    # MCP patterns
...
def find_patterns(messages: list[dict]) -> dict:
    ...
    for i, msg in enumerate(messages):
                ...
                for j in range(context_start, context_end):
                    if solution_regex.search(messages[j]["text"]):
                        ...
                        if not any(
                            ...
                            and p["error_context"][:100] == pair["error_context"][:100]
        ...
        for cmd in cmd_matches:
            if any(
                cmd.startswith(prefix) for prefix in ["python ", "pip ", "git ", "bd ", "claude "]
            ):
...
def format_for_memory_mcp(all_patterns: dict) -> dict:
    ...
    for from_entity, relation_type, to_entity in module_relations:
        if any(e["name"] == from_entity for e in entities) and any(
            e["name"] == to_entity for e in entities
        ):
            relations.append(
                {
                    "from": from_entity,
...
def main():
    ...
    print(
        f"  Technical patterns: {sum(len(v) for v in all_patterns['technical_patterns'].values())}"
    )
