...
class TestEmbeddingGenerator:
    ...
    def test_generator_initialization(self, mock_model):
        ...
        assert gen.backend == "sentence-transformers"
        mock_model.assert_called_once()

    def test_generator_with_prefix(self, mock_model):
        """Test generator stores prefix config."""
    ...
    def test_generate_embeddings_applies_document_prefix(self, mock_model):
        """Test that document prefix is applied during embedding generation."""
        import numpy as np
        ...
        call_args = mock_model.return_value.encode.call_args
        texts_passed = call_args[0][0]
        assert texts_passed == ["passage: Test text"]
    ...
    def test_embed_text_applies_query_prefix(self, mock_model):
        """Test that query prefix is applied to embed_text."""
        import numpy as np

        mock_model.return_value.encode.return_value = np.array([0.5] * 384)

        gen = EmbeddingGenerator(query_prefix="query: ")
        gen.embed_text("network analysis")

        call_args = mock_model.return_value.encode.call_args
        text_passed = call_args[0][0]
        assert text_passed == "query: network analysis"
...
class TestEmbeddingGeneratorOllama:
    """Tests for EmbeddingGenerator with Ollama backend."""
    ...
    def test_ollama_initialization(self, mock_ollama):
        ...
        mock_cls, mock_client = mock_ollama
        ...
        assert gen.embedding_dim == 4096
        assert gen.model is None  # No sentence-transformers model
        mock_cls.assert_called_once_with(host="http://localhost:11434")

    def test_ollama_embed_text(self, mock_ollama):
        """Test embedding a single text via Ollama."""
        ...
        embedding = gen.embed_text("test query")

        assert len(embedding) == 4096
        # Check the embed call (second call - first was the probe)
        last_call = mock_client.embed.call_args
        assert last_call.kwargs["input"] == ["test query"]

    def test_ollama_embed_text_with_prefix(self, mock_ollama):
        """Test that query prefix is applied with Ollama backend."""
        _, mock_client = mock_ollama

        gen = EmbeddingGenerator(
            model_name="qwen3-embedding:8b",
            backend="ollama",
            query_prefix="Instruct: Retrieve academic papers\nQuery: ",
        )

        gen.embed_text("network analysis")

        last_call = mock_client.embed.call_args
        assert last_call.kwargs["input"] == [
            "Instruct: Retrieve academic papers\nQuery: network analysis"
        ]

    def test_ollama_embed_batch(self, mock_ollama):
    ...
    def test_ollama_generate_embeddings(self, mock_ollama):
        ...
        result = gen.generate_embeddings(chunks, show_progress=False)
        assert len(result) == 2
        assert len(result[0].embedding) == 4096
        assert len(result[1].embedding) == 4096

    def test_ollama_connection_failure(self):
        """Test error handling when Ollama is unavailable."""
        with patch("src.indexing.embeddings._OllamaClient") as mock:
            client = MagicMock()
            client.embed.side_effect = ConnectionError("Connection refused")
            mock.return_value = client
